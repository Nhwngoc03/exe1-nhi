    ...
    <section className="py-20 px-6 md:px-8 bg-white relative">
      <div className="max-w-[900px] mx-auto animate-fade-in">
        ...
        <div className="text-center mb-16">
          ...
          <div className="bg-[#f0e8e9] border-l-4 border-[#6f0022] p-6 rounded-r-lg text-left max-w-2xl mx-auto mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {distortionReasons.map((reason) => {
              ...
              return (
                <div
                  key={reason.id}
                  className="border border-[#e1bec0] rounded-lg p-6 hover:shadow-md transition-all cursor-pointer bg-[#fafaf8]"
                  onClick={() => handleExpandExample(reason.id)}
                >
                  <div className="flex items-start gap-3 mb-4">
                    <IconComponent className="w-6 h-6 text-[#6f0022] flex-shrink-0 mt-1" />
                  ...
                  {reason.examples.length > 0 && (
                    <div className="space-y-3">
                      {reason.examples.map((example, idx) => (
                        <div key={idx} className="bg-white rounded p-4 border border-[#e1bec0]">
                          <div className="space-y-2 text-sm">
                            <div>
                              <span className="font-sans font-bold text-[#6f0022]">💭 Tình huống:</span>
                              <p className="text-[#3a3c3c] mt-1">{example.situation}</p>
                            </div>
                            <div>
                              <span className="font-sans font-bold text-[#6f0022]">❌ Nhận thức sai:</span>
                              <p className="text-[#3a3c3c] mt-1">{example.assumption}</p>
                            </div>
                            {expandedExample === reason.id && (
                              <>
                                <div>
                                  <span className="font-sans font-bold text-[#6f0022]">✓ Sự thật:</span>
                                  <p className="text-[#3a3c3c] mt-1">{example.reality}</p>
                                </div>
                                <div className="bg-[#ffd9dc] p-3 rounded">
                                  <span className="font-sans font-bold text-[#6f0022]">💡 Bài học:</span>
                                  <p className="text-[#6f0022] text-xs mt-1">{example.lesson}</p>
                                </div>
                              </>
                            )}
                          </div>
        ...
        <div>
          <div className="flex items-center gap-3 mb-8">
            <Zap className="w-8 h-8 text-[#6f0022] flex-shrink-0" />
            <h3 className="text-2xl sm:text-3xl font-extrabold text-[#1a1c1c] font-sans">
          ...
          <div className="space-y-6">
            {currentFactors.map((factor) => {
              ...
              return (
                <div
                  key={factor.id}
                  className="border-l-4 border-[#6f0022] bg-[#fafaf8] p-6 rounded-r-lg hover:shadow-md transition-all"
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="bg-[#ffd9dc] px-4 py-2 rounded flex items-start gap-2">
                        <Info className="w-4 h-4 text-[#6f0022] flex-shrink-0 mt-0.5" />
                        <p className="font-sans text-xs md:text-sm text-[#6f0022] font-semibold">
                          {factor.impact}
                        </p>
                      </div>
                    </div>
        ...
        {/* CTA Section */}
        <div className="mt-16 pt-12 border-t border-[#e1bec0] text-center">
          <div className="bg-[#f0e8e9] p-8 rounded-lg">
            <h4 className="font-sans font-bold text-lg text-[#1a1c1c] mb-3">
